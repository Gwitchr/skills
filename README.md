...
| [`agent-workflow-notifier`](agent-workflow-notifier/SKILL.md) | Slack direct messages from Claude Code hooks: ✅ when an agent finishes a turn, ❓ when it asks a question, 🔐 when it waits on a permission prompt, each with repo, branch, session, first prompt, and last message. Turns shorter than two minutes are skipped on the assumption that the owner was watching. One bash hook under `~/.claude/hooks`, one config file, one Slack app from the skill's manifest via the `slack` CLI, one hook block merged into `.claude/settings.local.json`. Posts with curl and a bot token cached at mode 600; falls back to the CLI only when the cache is empty and the CLI session expired. Pairs with `worktrees` through the `merge-json` copy mode. |

...
**Don't:** Inline imports, single-letter variables, nested ternaries, comments inferable from code, `&&` in JSX (use ternary + `null`), inline complex expressions in JSX props (extract to a named `const` first), type assertions (`as`), generic is-object type guards (`isRecord`, `isObject`), narrow by checking concrete properties instead (`"x" in value` plus `typeof` per field, at least two when the shape has two or more), `eslint-disable-next-line` comments, fix the root cause instead. Never hand-edit generated files (`routeTree.gen.ts`, `src/generated/prisma/**`, lockfiles). TS `enum` in app-authored code (Prisma-generated enums in `schema.prisma` are fine to consume).
